...
class Box(UIElement):
    def __init__(self, parent, pos, tr, colour):
        ...
        self.quad.SetVertices(self.absolute.bottom_left, self.absolute.top_right, utils.ui_level)
        self.Enable()
...
class TextBox(UIElement):
    ...
    def __init__(
        ...
        scale,
        colour=None,
        textType=texture.TextTypes.SCREEN_RELATIVE,
        alignment=texture.TextAlignments.LEFT,
    ):
        if tr is None:
    ...
    def Position(self, pos, scale, colour=None, ignore_height=False):
        ...
        self.lowest_y = 0
        row_height = (
            float(self.text_manager.font_height * self.scale * texture.global_scale) / self.absolute.size.y
        )
        ...
        letter_sizes = [
            Point(
                float(quad.width * self.scale * texture.global_scale) / self.absolute.size.x,
        ...
        while i < len(self.quads):
            ...
            self.SetLetterVertices(i, absolute_bl, absolute_tr, texture.TextTypes.LEVELS[self.text_type])
            if colour:
...
class FaderTextBox(TextBox):
    ...
    def Draw(self):
        ...
        glLoadIdentity()
        glTranslate(
            -self.tiles.viewpos.Get().x - self.tiles.width * gamedata.tile_dimensions.x,
            -self.tiles.viewpos.Get().y,
            0,
        )
        glTranslate(self.absolute.bottom_left.x, self.absolute.bottom_left.y, 0)
        glScale(self.draw_scale, self.draw_scale, 1)
...
class TextBoxButton(TextBox):
    ...
    def ReallocateResources(self):
        ...
        self.hover_quads = [utils.Quad(gamedata.ui_buffer) for i in range(4)]
