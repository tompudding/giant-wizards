...
class Viewpos(object):
    ...
    def SetTarget(self, point, t, rate=2):
        ...
        if self.tiles.dragging:
            return
        self.target = point
        self.target_change = utils.WrapDistance(
            self.target, self.pos, self.tiles.width * gamedata.tile_dimensions.x
        )
...
class TileHighlights(object):
    def __init__(self, tiles, size):
        self.tiles = tiles
        self.size = size
        self.quads = [utils.Quad(gamedata.quad_buffer, tc=self.tex_coords["highlight"]) for i in range(100)]
...
class Tiles(ui.UIRoot):
    def __init__(self, atlas, tiles_name, data_filename, map_size):
        ...
        self.cheats = (
            Cheat("manaplease", self, lambda x: x.AdjustMana(100)),
        ...
        self.control_box = ui.HoverableBox(
            gamedata.screen_root, Point(0.01, 0.07), Point(0.26, 0.29), (0, 0, 0, 0.6)
        )
        ...
        ui.TextBoxButton(
            self.control_box, "Centre", Point(0.35, 0.65), size=0.4, callback=self.CentreSelected
        )
        ...
        self.tex_coords = {}
        for name, (x, y, w, h) in data.items():
            top_left_x = float(x * gamedata.tile_dimensions.x) / self.atlas.Subimage(self.tiles_name).size.x
            top_left_y = (
                float(self.atlas.Subimage(self.tiles_name).size.y - y * gamedata.tile_dimensions.y)
                / self.atlas.Subimage(self.tiles_name).size.y
            )
            bottom_right_x = (
                top_left_x
                + float(w * gamedata.tile_dimensions.x) / self.atlas.Subimage(self.tiles_name).size.x
            )
            bottom_right_y = (
                top_left_y
                - float(h * gamedata.tile_dimensions.y) / self.atlas.Subimage(self.tiles_name).size.y
            )
            tc = numpy.array(
                (
            ...
            if name.endswith("_grass"):
                ...
                ends = [
                    end for end in ("_tree", "_scorched", "_mountain", "_water") if (name + end) not in data
                ]
                ends.append("_grass")
            else:
                ends = [""]
        ...
        self.text = ui.TextBox(
            parent=gamedata.screen_root, bl=Point(0.001, 0.001), tr=None, text="a", scale=0.5
        )
    ...
    def SetMapVertices(self, x_range=(0, None), y_range=(0, None)):
        ...
        if len(self.tile_quads) == 0:
            ...
            filling = False
        pos = 0
        for x in range(x_range[0], x_range[1] if x_range[1] is not None else len(self.map)):
            for y in range(y_range[0], y_range[1] if y_range[1] is not None else len(self.map[x])):
    ...
    def Draw(self):
        ...
        glDrawElements(
            GL_QUADS, gamedata.quad_buffer.current_size, GL_UNSIGNED_INT, gamedata.quad_buffer.indices
        )
        ...
        glDrawElements(
            GL_QUADS,
            gamedata.nonstatic_text_buffer.current_size,
            GL_UNSIGNED_INT,
            gamedata.nonstatic_text_buffer.indices,
        )
        ...
        glDrawElements(
            GL_QUADS, gamedata.colour_tiles.current_size, GL_UNSIGNED_INT, gamedata.colour_tiles.indices
        )
        ...
        glDrawElements(
            GL_QUADS,
            gamedata.mouse_relative_buffer.current_size,
            GL_UNSIGNED_INT,
            gamedata.mouse_relative_buffer.indices,
        )
        ...
        if self.hovered_ui:
            ...
            if button == 1 and not self.current_action:  # don't accept clicks while an actions happening
                ...
                if self.selected_player is None:
                    ...
                    if (
                        self.current_player.Controls(self.hovered_player)
                        and self.current_player.IsPlayer()
                        and (self.player_action is None or not self.player_action.FriendlyTargetable())
                    ):
                        # select them!
                        self.selected_player = self.current_player
                        self.selected_player.Select(self.hovered_player)
        ...
        if hovered_ui:
            ...
            if old_hovered != self.hovered_player:
                self.mouse_text.SetText(
                    self.hovered_player.name if self.hovered_player else " ", self.mouse_text_colour
                )
            ...
            if not self.viewpos.HasTarget():
                ...
                if action is not None:
                    if self.current_player.current_controlled not in (current_controlled, None):
                        ...
                        target = WorldCoords(self.current_player.current_controlled.pos) - (
                            gamedata.screen // 2
                        )
    ...
    def GameOver(self, winner):
        ...
        self.backdrop = ui.Box(gamedata.screen_root, Point(0.3, 0.3), Point(0.7, 0.7), (0, 0, 0, 0.6))
        self.win_message = ui.TextBox(
            parent=self.backdrop, bl=Point(0.125, 0.75), tr=None, text="%s wins!" % winner.name, scale=0.5
        )

        self.return_button = ui.TextBoxButton(
            self.backdrop, "Return", Point(0.375, 0.125), callback=self.Quit
        )
        self.InvalidateCache()
    ...
    def InvalidateCache(self):
        for path in list(self.pathcache.values()):
            path.Delete()
        for wizard in self.wizards:
    ...
    def PathTo(self, start, end):
        ...
        while True:
            ...
            if current == end:  # or len(Open) > 100:
                ...
                p = start, current
                path = utils.Path(current, self.tex_coords, len(self.map))
                if (p in self.pathcache and path.cost < self.pathcache[p].cost) or p not in self.pathcache:
...
def CreateTiles(player_states):
    map_size = (48, 24)
    tiles = Tiles(
        texture.TextureAtlas("tiles_atlas_0.png", "tiles_atlas.txt"), "tiles.png", "tiles.data", map_size
    )
    ...
    total_tried = 0
    player_list = [
        (player_states[i], colours[i], i) for i in range(len(colours)) if player_states[i] is not None
    ]
