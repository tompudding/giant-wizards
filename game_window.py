...
class Tiles(object):
    ...
    def GameOver(self, winner):
        ...
        self.backdrop = texture.BoxUI(
            Point(gamedata.screen.x * 0.3, gamedata.screen.y * 0.3),
        ...
        self.return_button = texture.TextButtonUI(
            "Return", Point(gamedata.screen.x * 0.45, gamedata.screen.y * 0.35), callback=self.Quit
        )
...
class GameWindow(object):
    def __init__(self, player_states):
        ...
        positions = []
        total_tried = 0
        players = [(player_states[i], names[i], i) for i in range(len(names)) if player_states[i] is not None]
        ...
        for i in range(len(players)):
            ...
            if player_states[type] is not None:
                self.tiles.AddWizard(pos=positions[i], type=type, isPlayer=isplayer, name=name)
