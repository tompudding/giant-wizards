    ...
    def Position(self, pos, scale):
        ...
        for (i, quad) in enumerate(self.quads):
            utils.setvertices(
                ...
                pos
                + Point(
                    (self.textmanager.font_width * (i + 1) * scale), self.textmanager.font_height * scale
                ),
                utils.text_level,
            )
            ...
            #                  gamedata.screen,
            #                  utils.text_level)
        self.top_right = pos + Point(
            (self.textmanager.font_width * len(self.quads) * scale), self.textmanager.font_height * scale
        )
...
class TextManager(object):
    ...
    def Letter(self, char, static):
        ...
        quad.tc[0:4] = numpy.array(
            ((left, bottom), (left, top), (right, top), (right, bottom)), numpy.float32
        )
        return quad
...
class BoxUI(UIElement):
    def __init__(self, pos, tr, colour):
        ...
        utils.setvertices(self.quad.vertex, self.bottom_left, self.top_right, utils.ui_level)
