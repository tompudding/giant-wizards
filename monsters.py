...
class AIMonsterChoices(object):
    def __init__(self, creators, caster):
        ...
        self.monsters.sort(key=lambda x: -x.desirability)
        print(caster.name, [(monster.monster.name, monster.desirability) for monster in self.monsters])


class Wizard(actor.Actor):
    initial_stats = actor.Stats(attack=4, defence=2, move=2, health=8, mana=2, ability_count=3)
    max_mana = 1000
    ...
    def __init__(self, pos, wizardType, tiles, playerType, name, player):
        ...
        if self.IsPlayer():
            self.action_choices = action.ActionChoiceList(
                self.options_box,
                self,
                Point(0, 0),
                Point(1, 0.7),
                (
                    ...
                    action.BlastActionCreator(
                        self,
                        [
                            action.WeakWizardBlastAction,
                            action.WizardBlastAction,
                            action.PowerfulWizardBlastAction,
                            action.EpicWizardBlastAction,
                        ],
                    ),
                    action.SummonActionCreator(
                        self,
                        [
                            SummonGoblinRuntAction,
                    ...
                    action.SummonActionCreator(self, [SummonJuvenileDragonAction, SummonRedDragonAction]),
                    action.TeleportActionCreator(self, [action.TeleportAction, action.RefinedTeleportAction]),
                ),
            )
            ...
            self.blast_action_creator = action.BlastActionCreator(
                self,
                [
                    action.WeakWizardBlastAction,
                    action.WizardBlastAction,
            ...
            self.summon_goblin_creator = action.SummonActionCreator(
                self,
                [
                    SummonGoblinRuntAction,
                    SummonGoblinWarriorAction,
            ...
            self.teleport_action_creator = action.TeleportActionCreator(
                self, [action.TeleportAction, action.RefinedTeleportAction]
            )
            self.move_action_creator = action.MoveActionCreator(self, [action.MoveAction])
            self.ai_actions = [
                self.blast_action_creator,
                self.summon_goblin_creator,
                self.move_action_creator,
            ]
    ...
    def TakeAction(self, t):
                    ...
                    if self.stats.move > 0:
                        ...
                        if self.move_action_creator.Valid(away_path.steps[0]):
                            self.action_list.extend(
                                self.move_action_creator.Create(away_path.steps[0], t, self)
                            )
                        ...
                        if danger_score > 2 and self.player.personality.confidence < 4:
                            ...
                            if target_action is not None:
                                ...
                                if offset is not None:
                                    # Woop can teleport
                                    self.action_list.extend(
                                        self.teleport_action_creator.Create(offset, t, self)
                                    )
                    ...
                    if target_action is not None:
                        ...
                        for adjust in utils.Spiral(16):
                            if self.teleport_action_creator.Valid(offset + adjust):
                                self.action_list.extend(
                                    self.teleport_action_creator.Create(offset + adjust, t, self)
                                )
                                return self.action_list.pop(0)
                            break
        ...
        if len(self.action_list) == 0:
            if (
                self.saving_for
                and self.saving_for.cost + self.player.personality.emergency_mana <= self.stats.mana
            ):
        ...
        if len(self.action_list) == 0:
            ...
            if (
                len(self.player.controlled) < self.player.personality.necessary_monsters
                and danger_score > self.player.personality.confidence / 4
            ):
                ...
                required = sorted(
                    [
                        (monster, monster.desirability - actual_ratio)
                        for actual_ratio, monster in zip(actual_ratios, self.monsters.monsters)
                    ],
                    key=lambda x: -x[1],
                )
    ...
    def SummonMonster(self, creator, t):
        choices = sorted([p for p in creator.valid_vectors], key=lambda x: x.length())
        for point in choices:
    ...
    def ReadyToAttack(self):
        if (
            len(self.player.controlled) > self.player.personality.necessary_monsters
            and self.stats.health > 20.0 / self.player.personality.confidence
            and self.stats.mana > 10.0 / self.player.personality.confidence
        ):
            return True
        return False
...
class FlyingMonster(Monster):
    ...
    def PathTo(self, pos):
        targets = [
            (self.pos + p, (pos - (self.pos + p)).length()) for p in self.move_action_creator.valid_vectors
        ]
        if len(targets) == 0:
            return None
...
class GoblinRunt(Monster):
    initial_stats = actor.Stats(attack=2, defence=2, move=1, health=3, mana=0)
    name = "Goblin Runt"


class GoblinWarrior(Monster):
    initial_stats = actor.Stats(attack=4, defence=3, move=3, health=6, mana=0)
    name = "Goblin Warrior"


class GoblinShaman(BlastingMonster):
    initial_stats = actor.Stats(attack=2, defence=1, move=2, health=4, mana=1, ability_count=2)
    max_mana = 6
    name = "Goblin Shaman"
    actionchoice_list = [
        (action.MoveActionCreator, [action.MoveAction]),
...
class GoblinLord(Monster):
    initial_stats = actor.Stats(attack=6, defence=4, move=4, health=14, mana=0)
    name = "Goblin Lord"
...
class SummonGoblinRuntAction(SummonGoblinAction):
    ...
    stats = [
        (stat_name, getattr(GoblinRunt.initial_stats, stat_name))
        for stat_name in ("attack", "defence", "move", "health", "mana")
    ]
...
class SummonGoblinWarriorAction(SummonGoblinAction):
    ...
    stats = [
        (stat_name, getattr(GoblinWarrior.initial_stats, stat_name))
        for stat_name in ("attack", "defence", "move", "health", "mana")
    ]
...
class SummonGoblinShamanAction(SummonGoblinAction):
    description = (
        "Goblins are not usually magical by nature, so this one is something of a rarity. Use it well"
    )
    cost = 5
    ...
    stats = [
        (stat_name, getattr(GoblinShaman.initial_stats, stat_name))
        for stat_name in ("attack", "defence", "move", "health", "mana")
    ]
...
class SummonGoblinLordAction(SummonGoblinAction):
    ...
    stats = [
        (stat_name, getattr(GoblinLord.initial_stats, stat_name))
        for stat_name in ("attack", "defence", "move", "health", "mana")
    ]
...
class Brontosaurus(Dinosaur):
    initial_stats = actor.Stats(attack=0, defence=30, move=1, health=30, mana=0)
    name = "Brontosaurus"
...
class Velociraptor(Dinosaur):
    initial_stats = actor.Stats(attack=6, defence=0, move=4, health=3, mana=0)
    name = "Velociraptor"


class TRex(Dinosaur):
    initial_stats = actor.Stats(attack=9, defence=8, move=4, health=21, mana=0)
    name = "T-Rex"


class Pteranodon(Dinosaur, FlyingMonster):
    initial_stats = actor.Stats(attack=4, defence=4, move=5, health=5, mana=0)
    name = "Pteranodon"
...
class JuvenileDragon(BlastingMonster, FlyingMonster):
    initial_stats = actor.Stats(attack=6, defence=7, move=4, health=14, mana=0, ability_count=1)
    name = "Juvenile Dragon"
    ability_name = "Dragon Flame"

    actionchoice_list = [
        (action.FlyActionCreator, [action.FlyAction]),
...
class RedDragon(BlastingMonster, FlyingMonster):
    initial_stats = actor.Stats(attack=10, defence=10, move=5, health=28, mana=0, ability_count=1)
    name = "Red Dragon"
    ability_name = "Dragon Flame"
...
class SummonJuvenileDragonAction(SummonDragonAction):
    ...
    monster_type = "dragon"
    stats = [
        (stat_name, getattr(JuvenileDragon.initial_stats, stat_name))
        for stat_name in ("attack", "defence", "move", "health", "mana")
    ] + [("special", "flying, dragon flame")]
...
class SummonRedDragonAction(SummonDragonAction):
    ...
    monster_type = "dragon"
    stats = [
        (stat_name, getattr(RedDragon.initial_stats, stat_name))
        for stat_name in ("attack", "defence", "move", "health", "mana")
    ] + [("special", "flying, dragon flame")]
...
class SummonVelociraptorAction(SummonDinosaurAction):
    ...
    stats = [
        (stat_name, getattr(Velociraptor.initial_stats, stat_name))
        for stat_name in ("attack", "defence", "move", "health", "mana")
    ]
...
class SummonPteranodonAction(SummonDinosaurAction):
    ...
    stats = [
        (stat_name, getattr(Pteranodon.initial_stats, stat_name))
        for stat_name in ("attack", "defence", "move", "health", "mana")
    ] + [("special", "Flying")]
...
class SummonBrontosaurusAction(SummonDinosaurAction):
    ...
    stats = [
        (stat_name, getattr(Brontosaurus.initial_stats, stat_name))
        for stat_name in ("attack", "defence", "move", "health", "mana")
    ]
...
class SummonTRexAction(SummonDinosaurAction):
    ...
    monster_type = "trex"
    stats = [
        (stat_name, getattr(TRex.initial_stats, stat_name))
        for stat_name in ("attack", "defence", "move", "health", "mana")
    ]
