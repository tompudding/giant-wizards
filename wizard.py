...
wizard_types = ["purple_wizard", "red_wizard", "yellow_wizard", "green_wizard"]
    ...
    def Update(self, t):
        ...
        if self.firing:
            if t > self.end_time:
                ...
                utils.setvertices(
                    self.quad.vertex, utils.WorldCoords(pos), utils.WorldCoords(pos + Point(1, 1)), 0.5
                )
                return False
...
class Wizard(object):
    def __init__(self, pos, type, tiles, isPlayer, name):
        ...
        self.options_box = texture.BoxUI(
        ...
        self.action_points_text = texture.TextObject(
            "Action Points : %d" % self.action_points, gamedata.text_manager
        )
        self.action_points_text.Position(Point(gamedata.screen.x * 0.7, gamedata.screen.y * 0.87), 0.33)
        self.action_header = texture.TextObject(
            "%s%s" % ("Action".ljust(14), "Cost".rjust(6)), gamedata.text_manager
        )
        ...
        self.health_text.Position(utils.WorldCoords(Point(self.pos.x + 0.6, self.pos.y + 0.8)), 0.3)
        self.options_box.Disable()
        for t in self.static_text:
            t.Disable()
        for a in self.action_choices:
            a.Disable()
        self.end_turn = texture.TextButtonUI(
            "End Turn", Point(gamedata.screen.x * 0.72, gamedata.screen.y * 0.07), callback=self.EndTurn
        )
    ...
    def SetPos(self, pos):
        ...
        utils.setvertices(
            self.quad.vertex,
            utils.WorldCoords(self.pos),
            utils.WorldCoords(self.pos) + gamedata.tile_dimensions,
            0.5,
        )
    ...
    def TakeAction(self, t):
        if self.action_list is None:
            ...
            for wizard in self.tiles.wizards:
                ...
                if match[0] is None or distance < match[0]:
