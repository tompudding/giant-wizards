...
class Path(object):
    ...
    def __init__(self, start, tc, width, reversed=False):
        ...
        self.steps = [WrapDistance(self.path[i + 1], self.path[i], width) for i in range(len(self.path) - 1)]
        self.quads = []
        self.tc = tc
        self.width = width
        self.cost = start.g
    ...
    def Enable(self):
        if self.quads:
            ...
            for pos, direction in zip(self.path, self.steps):
                ...
                tr = bl + Point(1, 1)
                q.SetVertices(WorldCoords(bl), WorldCoords(tr), 10)
                self.quads.append(q)
    ...
    def Segments(self):
        steps = [self.path[i + 1] - self.path[i] for i in range(len(self.path) - 1)]
        last = None
        length = 1
        while steps:
            a = steps.pop(0)
...
def Ratios(n):
    if n < 2:
        return [1]
    points = sorted([0, 1] + [random.random() for i in range(n - 1)])
